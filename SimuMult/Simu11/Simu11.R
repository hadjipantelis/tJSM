...
Data <- MultData11[[i]]
fitLME <- lme(Y ~ bs(ObsTime, knots = 2, Boundary.knots = c(0, 7)), random =~ 1 | ID, data = Data)
fitCOX <- coxph(Surv(start, stop, event) ~ Z, data = Data, x = TRUE)

time1 <- system.time(fit1 <- jmodelMult(fitLME, fitCOX, Data, rho = 1, control = control1))[3]
time2 <- system.time(fit2 <- jmodelMult(fitLME, fitCOX, Data, rho = 1, control = control2))[3]
time3 <- system.time(fit3 <- jmodelMult(fitLME, fitCOX, Data, rho = 1, control = control3))[3]
...
save(results, file = paste("MultResult11", i, ".RData", sep = ""))
