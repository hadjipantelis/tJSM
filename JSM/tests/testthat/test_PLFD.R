...
test_that(" basic PLFD jmodelMult test with for aids data model = 1, rho = 0 ", { 
  ...
  expect_equal( mean (m_MULT$Vcov), 0.00050059556321931, tolerance = (10^12)*myEps, scale = 1)
})
...
fitCOX <- coxph(Surv(start, stop, event) ~ drug, data = aids, x = TRUE)
control <- list(nknot = 5, SE.method = 'PLFD', tol.L = 1e-04, tol.P = 1e-05)
...
test_that(" basic PLFD jmodelTM test with for aids data model = 1, rho = 0 ", {
  ...
  expect_equal( mean (m_TM$est.bi), -0.000211665949028 , tolerance = (10^14)*myEps, scale = 1)
  expect_equal( mean (m_TM$Vcov), 0.000410999416305 , tolerance = (10^13)*myEps, scale = 1)
})
...
test_that(" basic PLFD jmodelTM test with for aids data model = 2, rho = 1 ", {
  ...
  expect_equal( mean (m_TM$est.bi), -0.000700231727253, tolerance = (10^14)*myEps, scale = 1)
  expect_equal( mean (m_TM$Vcov), 0.000683913958988, tolerance = (10^13)*myEps, scale = 1)
})
