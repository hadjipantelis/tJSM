...
test_that(" basic PFDS jmodelMult test with for aids data model = 1, rho = 0 ", { 
  ...
  expect_equal( mean (m_MULT$Vcov),  0.00051055462959423, tolerance = (10^11)*myEps, scale = 1)
})
...
fitCOX <- coxph(Surv(start, stop, event) ~ drug, data = aids, x = TRUE)
control <- list(nknot = 6, SE.method = 'PFDS', tol.L = 1e-08, tol.P = 1e-05)
...
test_that(" basic PLFD jmodelTM test with for aids data model = 1, rho = 0 ", {
  ...
  expect_equal( mean (m_TM$Vcov),  0.000407428109278, tolerance = (10^7)*myEps, scale = 1)
})
