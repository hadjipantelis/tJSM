...
DQfuncGeneric <- function (model, ptheta, theta, n, Z.st, Y.st, X.st, Ztime, nk, Wtime, Ztime.b, Wtime2, Xtime, Xtime2, GQ, Index, Index1, rho, d, wGQ, ncx, ncw, p, ncz, ncz2, b, Ztime2.st, Index0, X, Y, ID, N, Index2, Z) { # ptheta means "theta prime"
  ...
  if (model==2){
    ...
    exp.es <- as.numeric(Wtime2 %*% phi) + alpha*Ztime2.b  
  } else if(model==1){
  ...
  if (model==2){
    exp.esp <-as.numeric(Wtime2 %*% pphi) +   alpha *Ztime2.b # M*GQ matrix #
  } else if(model==1){
    exp.esp <- as.vector(Wtime2 %*% pphi + palpha * Xtime2 %*% pbeta) +  alpha *Ztime2.b # M*GQ matrix #
