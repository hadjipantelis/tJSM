...
InitValTMGeneric <- function (beta,model,n,X,Z,bBLUP, ID, Xtime, Ztime, Xtime2, Ztime2, Indcs, start, event, stop, W , ncw, Wtime2, rho, nk, Wtime22, d, Wtime, cvals) {
  ...
  fit <- coxph(Surv(start, stop, event) ~ W + fixedOrRand, data = data.init) 
  phi.old <- fit$coefficients[1:ncw]
  alpha.old <- fit$coefficients[ncw + 1]
  temp <- as.vector(exp(Wtime2 %*% phi.old + alpha.old * fixedOrRand.time2)) # M*1 vector #
  ...
  if (rho == 0) {
    ...
    for (it in 1:iter) {
  exp.es <- exp(as.vector(Wtime2 %*% phi.old + alpha.old * fixedOrRand.time2))
  const <- rep(0, n)
  ...
  exp.esn <- exp(as.vector(Wtime2 %*% phi.new + alpha.new * fixedOrRand.time2))
