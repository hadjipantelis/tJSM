...
PRES <- function (model, theta, tol, iter, delta) {     
  ...
  for (i in 1:len) {
    ...
    theta.input1 <- list(beta = list1$beta, phi = list1$phi, alpha = list1$alpha, 
                         Ysigma = list1$Ysigma, BSigma = list1$BSigma, lamb = result1$lamb)
    theta.input2 <- list(beta = list2$beta, phi = list2$phi, alpha = list2$alpha, 
                         Ysigma = list2$Ysigma, BSigma = list2$BSigma, lamb = result2$lamb)
    theta.input3 <- list(beta = list3$beta, phi = list3$phi, alpha = list3$alpha, 
                         Ysigma = list3$Ysigma, BSigma = list3$BSigma, lamb = result3$lamb)
    theta.input4 <- list(beta = list4$beta, phi = list4$phi, alpha = list4$alpha, 
