...
EMiterMultGeneric <- function (theta.old) { # Use apply instead of matrix calculation #
  ...
  if (model==1){ 
    ...
    exp.es <- as.vector(Ztime2 %*% phi.old) + alpha.old * Btime2.b # M*nknot matrix #
  } else { 
    log.density1 <- log.lamb + as.vector(Ztime %*% phi.old) + alpha.old * bi # n*nknot matrix #
    exp.es <- as.vector(Ztime2 %*% phi.old) + alpha.old * bi[Index, ] # M*nknot matrix #
  }
  ...
  if (model ==1) {
    ...
    eta.s.n2 <- as.vector(Ztime2 %*% phi.new) + alpha.new * bi[Index, ] # M*nknot matrix #
  }
