...
#=============== Initial Value Calculation for Model I with NMRE ===============#

InitValMult1 <- function (gamma) {
  ...
  #========== fit the Cox model ==========#
  fixed <- bBLUP[ID] * as.vector(B %*% gamma)
  fixed.time <- bBLUP * as.vector(Btime %*% gamma)
  fixed.time2 <- bBLUP[Index] * as.vector(Btime2 %*% gamma)
  data.init <- data.frame(start = start, stop = stop, event = event, Z = Z, fixed = fixed)
  fit <- coxph(Surv(start, stop, event) ~ Z + fixed, data = data.init) 
  phi.old <- fit$coefficients[1 : ncz]
  alpha.old <- fit$coefficients[ncz + 1]
  temp <- as.vector(exp(Ztime2 %*% phi.old + alpha.old * fixed.time2)) # M*1 vector #
  ...
  if (rho == 0) {
    ...
    for (it in 1:iter) {
      exp.es <- exp(as.vector(Ztime2 %*% phi.old + alpha.old * fixed.time2))
      const <- rep(0, n)
      ...
      temp2 <- sum(CondExp2 * as.vector(tapply(fixed.time2 * exp.es * lamb.old[Index1], Index, sum))) # scalar #
      ...
      temp4 <- sum(CondExp2 * as.vector(tapply(fixed.time2 ^ 2 * exp.es * lamb.old[Index1], Index, sum))) # scalar #
      temp5 <- lapply(1:ncz, function(i) CondExp2 * as.vector(tapply(Ztime2[, i] * fixed.time2 * exp.es * 
                                         lamb.old[Index1], Index, sum)))
      temp5 <- sapply(temp5, sum) # vector of length ncz #
      
      phiScore <- colSums(d * Ztime) - temp1 # vector of length ncz #
      alphaScore <- sum(d * fixed.time) - temp2
      ...
      exp.esn <- exp(as.vector(Ztime2 %*% phi.new + alpha.new * fixed.time2))
